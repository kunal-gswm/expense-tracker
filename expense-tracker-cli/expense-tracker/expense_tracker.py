...
def edit_expense(expenses):
    ...
    while True:
        id_str = input("\nEnter the ID to edit (or 0 to cancel): ").strip()
        try:
            exp_id = int(id_str)
            break
        except ValueError:
    ...
    target = next((e for e in expenses if e["id"] == exp_id), None)
    if target is None:
        print(f"  ⚠  No expense found with ID {exp_id}.")
        return

    print(f"\n  Editing: [{target['id']}] {target['description']}  ₹{target['amount']:.2f}  [{target['category']}]")
    print("  (Press Enter to keep the current value)\n")

    new_desc = input(f"  Description [{target['description']}]: ").strip()
    if new_desc:
    ...
    while True:
        new_amt = input(f"  Amount      [₹{target['amount']:.2f}]: ").strip()
        if not new_amt:
            break
        try:
...
def main():
    ...
    menu = {
        ...
        "5": ("Summary / Total",   lambda: show_summary(expenses)),
        "6": ("Exit",              None),
    }
